...
public:
  ...
  Eigen::Affine3d refineCameraPose()
  {
    ...
    tf_listener_.waitForTransform(base_cf_, "left_hand", ros::Time(0), ros::Duration(1.0));
    tf_listener_.lookupTransform(base_cf_, "left_hand", ros::Time(0), rh_transform);
    ...
    //Eigen::Affine3d qr_rotation = Eigen::Affine3d(Eigen::Vector3d::UnitZ() * Eigen::AngleAxisd(M_PI, Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd((M_PI / 2.0), Eigen::Vector3d::UnitZ()));
    Eigen::Affine3d qr_rotation = Eigen::Affine3d(Eigen::AngleAxisd(-M_PI / 2.0, Eigen::Vector3d(0, 0, 1)));
