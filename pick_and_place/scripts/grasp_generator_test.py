...
class grasp_generator(object):
    ...
    def broadcast_frame(self,msg):
        ...
        cube_frames = [cf for cf in frames if 'unknown' in cf]
        for cube_frame in cube_frames:
            if self.listen.frameExists("/root") and self.listen.frameExists(cube_frame):
                translation, quaternion = self.listen.lookupTransform("/root", cube_frame, rospy.Time(0))
                # Identity matrix. Set the requ rot n trans wrt obj frame
                requrd_rot = (np.pi,0,0) # in radians
                requrd_trans = (0,.035,0.09+translation[2]+(float(cube_frame[-1])*0.04))
                ...
                self.broadcast.sendTransform(trans_1, quat_1,
                                        rospy.Time.now(),
                                        "/cube_{}_grasp".format(cube_frame[-1]),
                                        "/root")
